...
class AuthorCreateViewTest(TestCase):
    ...
    def setUp(self):
        ...
        test_user_1 = User.objects.create_user(
            username='test1', password='12121212')
        test_user_2 = User.objects.create_user(
            username='test2', password='12121212')
        ...
        test_user_1.user_permissions.add(permAddAuthor)
        test_user_1.save()
    ...
    def test_forbidden_if_logged_in_but_no_permission(self):
        login = self.client.login(username='test2', password='12121212')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 403)

    def test_logged_in_with_permission(self):
        login = self.client.login(username='test1', password='12121212')
        response = self.client.get(reverse('author-create'))
        self.assertEqual(response.status_code, 200)
    ...
    def test_redirects_to_author_detail_on_success(self):
        login = self.client.login(username='test1', password='12121212')
        response = self.client.post(reverse('author-create'), {'first_name': 'John', 
                                                                'last_name': 'Doe', 
                                                                'date_of_birth': '01/01/1970', 
                                                                'date_of_death': '11/11/2023'
                                                                })
        self.assertEqual(response.status_code, 302)
